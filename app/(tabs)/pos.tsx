...
export default function POSScreen() {
  ...
  if (layout.isWide) {
    return (
      <View style={[styles.root, { backgroundColor: colors.background }]}>
        <Sidebar visible={sidebarOpen} activeScreen="pos" onClose={() => setSidebarOpen(false)} onNavigate={(s) => { const r = TAB_ROUTES[s]; if (r) router.push(r as any); }} />
        ...
        <View style={{ flex: 1, flexDirection: "row" }}>
          ...
          <View style={{ flex: 1, flexDirection: "column", overflow: "hidden" }}>
            ...
            <View style={[styles.header, { paddingTop: topPad + 8, backgroundColor: colors.primary }]}>
              <Text style={[styles.headerTitle, { fontFamily: "Inter_700Bold" }]}>
                {editingDelivery ? `Editing ${editingDelivery.orderNo}` : "Point of Sale"}
              </Text>
              <View style={{ flexDirection: "row", gap: 6 }}>
                <TouchableOpacity onPress={() => router.push("/delivery" as any)} style={[styles.heldBtn, { backgroundColor: deliveryCount > 0 ? "rgba(255,255,255,0.22)" : "rgba(255,255,255,0.1)" }]}>
                  <Feather name="truck" size={14} color={deliveryCount > 0 ? "#fff" : "rgba(255,255,255,0.65)"} />
                  <Text style={[styles.heldBtnText, { fontFamily: "Inter_600SemiBold", color: deliveryCount > 0 ? "#fff" : "rgba(255,255,255,0.65)" }]}>{deliveryCount > 0 ? `Delivery (${deliveryCount})` : "Delivery"}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setHeldPanelVisible(true)} style={[styles.heldBtn, { backgroundColor: heldCount > 0 ? "rgba(255,255,255,0.22)" : "rgba(255,255,255,0.1)" }]}>
                  <Feather name="pause-circle" size={14} color={heldCount > 0 ? "#fff" : "rgba(255,255,255,0.65)"} />
                  <Text style={[styles.heldBtnText, { fontFamily: "Inter_600SemiBold", color: heldCount > 0 ? "#fff" : "rgba(255,255,255,0.65)" }]}>{heldCount > 0 ? `Held (${heldCount})` : "Held"}</Text>
            ...
            {editingDelivery && (
              <View style={[styles.editBanner, { backgroundColor: "#4F46E5" }]}>
                ...
                <Text style={[styles.editBannerText, { fontFamily: "Inter_600SemiBold" }]}>Editing {editingDelivery.orderNo} for {editingDelivery.customer.name}</Text>
                <TouchableOpacity onPress={() => { setEditingDelivery(null); setCart([]); }}><Feather name="x" size={15} color="rgba(255,255,255,0.8)" /></TouchableOpacity>
              </View>
            )}
            {deliverySaved && (
              <View style={[styles.editBanner, { backgroundColor: "#10B981" }]}>
                <Feather name="check-circle" size={13} color="#fff" />
                <Text style={[styles.editBannerText, { fontFamily: "Inter_600SemiBold" }]}>Delivery order saved! Track it in Delivery Orders.</Text>
              </View>
            )}
            {/* Search */}
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.catTabs} style={styles.catTabsWrap}>
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <TouchableOpacity key={cat} onPress={() => setActiveCategory(cat)} style={[styles.catTab, { backgroundColor: isActive ? colors.primary : colors.card, borderColor: isActive ? colors.primary : colors.border }]}>
                    <Text style={[styles.catTabText, { color: isActive ? "#fff" : colors.mutedForeground, fontFamily: isActive ? "Inter_600SemiBold" : "Inter_400Regular" }]}>{cat}</Text>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
          ...
          {/* ── Right: Cart Panel ── */}
          <View style={{ width: 340, borderLeftWidth: 1, borderLeftColor: colors.border, backgroundColor: colors.card, flexDirection: "column" }}>
            {/* Cart header */}
            <View style={{ paddingHorizontal: 16, paddingTop: topPad + 12, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: colors.border, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                ...
                <Text style={{ color: colors.foreground, fontFamily: "Inter_700Bold", fontSize: 17 }}>
                  Cart{hasCart ? ` (${cart.length})` : ""}
                </Text>
              </View>
              {hasCart && (
                <TouchableOpacity onPress={() => setCart([])} style={{ paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, backgroundColor: "#EF444415" }}>
                  <Text style={{ color: colors.destructive, fontFamily: "Inter_500Medium", fontSize: 12 }}>Clear</Text>
                </TouchableOpacity>
              )}
            </View>
            ...
            {hasCart ? (
              <>
                <ScrollView style={styles.itemList} showsVerticalScrollIndicator={false} nestedScrollEnabled>
                  {cart.map((item, idx) => (
                    <View key={item.id} style={[styles.itemRow, idx < cart.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}>
                      <View style={{ flex: 1 }}>
                        <Text style={[styles.itemName, { color: colors.foreground, fontFamily: "Inter_500Medium" }]} numberOfLines={1}>{item.name}</Text>
                        <Text style={[styles.itemUnit, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>₹{item.price}/{item.unit}</Text>
                      </View>
                      {item.weightBased ? (
                        <TouchableOpacity style={[styles.weightEditBtn, { borderColor: "#4F46E5", backgroundColor: "#4F46E510" }]} onPress={() => { const prod = allProducts.find(p => p.id === item.id); if (prod) setWeightModal(prod); }}>
                          <Feather name="sliders" size={11} color="#4F46E5" />
                          <Text style={[styles.weightEditText, { color: "#4F46E5", fontFamily: "Inter_700Bold" }]}>{formatQty(item.qty, item.unit)}</Text>
                        </TouchableOpacity>
                      ) : (
                        <View style={styles.qtyRow}>
                          <TouchableOpacity onPress={() => updatePcsQty(item.id, -1)} style={[styles.qtyBtn, { borderColor: colors.border }]}>
                          ...
                          <TouchableOpacity onPress={() => updatePcsQty(item.id, 1)} style={[styles.qtyBtn, { borderColor: colors.primary, backgroundColor: colors.primary }]}>
                            <Feather name="plus" size={12} color="#fff" />
                          </TouchableOpacity>
                        </View>
                      )}
                      <Text style={[styles.itemAmt, { color: colors.foreground, fontFamily: "Inter_600SemiBold" }]}>₹{(item.price * item.qty).toFixed(item.weightBased ? 2 : 0)}</Text>
                ...
                <TouchableOpacity style={[styles.holdOrderBtn, { borderColor: "#F59E0B50", backgroundColor: "#F59E0B0C" }]} onPress={() => setHoldModalVisible(true)} activeOpacity={0.75}>
                  <Feather name="pause-circle" size={15} color="#D97706" />
                  <Text style={[styles.holdOrderText, { color: "#D97706", fontFamily: "Inter_600SemiBold" }]}>Hold Order</Text>
                </TouchableOpacity>
                ...
                >
                  ...
                  <Text style={[styles.proceedText, { fontFamily: "Inter_700Bold" }]}>
                    {editingDelivery ? `Save · ₹${total.toLocaleString()}` : `Pay · ₹${total.toLocaleString()}`}
                  </Text>
                </TouchableOpacity>
              </>
            ) : (
              <View style={{ flex: 1, alignItems: "center", justifyContent: "center", gap: 14, padding: 28 }}>
                ...
                <Text style={{ color: colors.foreground, fontFamily: "Inter_600SemiBold", fontSize: 17 }}>Cart is empty</Text>
                <Text style={{ color: colors.mutedForeground, fontFamily: "Inter_400Regular", fontSize: 13, textAlign: "center", lineHeight: 20 }}>
                  Browse products on the left and tap to add them here
                </Text>
              </View>
            )}
          </View>
