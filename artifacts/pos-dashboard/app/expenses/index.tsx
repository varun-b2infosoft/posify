...
function DeleteConfirmModal({ visible, onClose, onConfirm, colors }: {
  visible: boolean; onClose: () => void; onConfirm: () => void; colors: any;
}) {
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.dialogOverlay}>
        <View style={[styles.dialogBox, { backgroundColor: colors.card }]}>
          ...
          <Text style={[styles.dialogTitle, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>Delete Expense?</Text>
          ...
          <View style={{ flexDirection: "row", gap: 10, marginTop: 4 }}>
            ...
            <TouchableOpacity style={[styles.deleteBtn, { backgroundColor: "#EF4444" }]} onPress={onConfirm}>
              <Text style={{ color: "#fff", fontFamily: "Inter_700Bold" }}>Delete</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
...
export default function ExpensesScreen() {
  ...
  const confirmDelete = () => {
    if (deleteId) { deleteExpense(deleteId); setDeleteId(null); }
  };
  ...
  const AddModal = () => (
    <Modal visible={modal} animationType={layout.isWide ? "fade" : "slide"} transparent onRequestClose={() => setModal(false)}>
      <View style={[styles.modalOverlay, layout.isWide && styles.modalOverlayCenter]}>
        <KeyboardAvoidingView
          behavior={Platform.OS === "ios" ? "padding" : undefined}
          style={layout.isWide ? { width: "100%", maxWidth: 480 } : { width: "100%" }}
        >
          <View style={[
            styles.modalSheet,
            layout.isWide && styles.modalDialog,
    ...
    return (
      <View style={[styles.item, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <View style={{ alignItems: "flex-end", gap: 6 }}>
          ...
          <TouchableOpacity onPress={() => setDeleteId(e.id)}>
    ...
    return (
      <View style={[styles.root, { backgroundColor: colors.background }]}>
        ...
        <View style={[dk.header, { backgroundColor: "#4F46E5", paddingTop: topPad }]}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[dk.headerTitle, { fontFamily: "Inter_700Bold" }]}>Expenses</Text>
            <Text style={[dk.headerSub, { fontFamily: "Inter_400Regular" }]}>This month: {fmt(monthly)}</Text>
          </View>
          <TouchableOpacity style={[dk.addBtn, { backgroundColor: "rgba(255,255,255,0.2)" }]} onPress={() => setModal(true)}>
            <Feather name="plus" size={16} color="#fff" />
            <Text style={[dk.addBtnText, { fontFamily: "Inter_600SemiBold" }]}>Add Expense</Text>
          </TouchableOpacity>
        </View>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: botPad + 24 }}>
          <View style={dk.body}>
            ...
            {/* Left: stats + category breakdown */}
            <View style={dk.leftCol}>
              <View style={[dk.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <Text style={[dk.cardTitle, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>Monthly Summary</Text>
                <View style={{ alignItems: "center", paddingVertical: 8 }}>
                  <Text style={[{ color: "#EF4444", fontFamily: "Inter_700Bold", fontSize: 32 }]}>{fmt(monthly)}</Text>
                  <Text style={[{ color: colors.mutedForeground, fontFamily: "Inter_400Regular", fontSize: 13, marginTop: 4 }]}>Total this month</Text>
                </View>
                <View style={[{ height: 1, backgroundColor: colors.border }]} />
                <Text style={[dk.cardTitle, { color: colors.foreground, fontFamily: "Inter_700Bold", marginTop: 4 }]}>By Category</Text>
                {EXPENSE_CATEGORIES.map(c => (
                  <View key={c} style={dk.catRow}>
                    <View style={[dk.catDot, { backgroundColor: EXPENSE_CAT_COLORS[c] }]} />
                    <Text style={[dk.catName, { color: colors.foreground, fontFamily: "Inter_500Medium" }]}>{c}</Text>
                    <Text style={[dk.catVal, { color: EXPENSE_CAT_COLORS[c], fontFamily: "Inter_700Bold" }]}>{fmt(catTotals[c])}</Text>
                  </View>
                ))}
              </View>
            </View>
            ...
            <View style={dk.rightCol}>
              {/* Inline filter chips */}
              <View style={[dk.filterRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <Feather name="filter" size={14} color={colors.mutedForeground} style={{ marginRight: 4 }} />
                {CATS_WITH_ALL.map(c => (
                  <TouchableOpacity
                    key={c}
                    style={[styles.chip, {
                      borderColor: filter === c ? (c === "All" ? "#4F46E5" : EXPENSE_CAT_COLORS[c as ExpenseCategory]) : colors.border,
                      backgroundColor: filter === c ? (c === "All" ? "#4F46E5" : EXPENSE_CAT_COLORS[c as ExpenseCategory]) : "transparent",
                    }]}
                    onPress={() => setFilter(c)}
                  >
              ...
              <View style={{ gap: 8 }}>
                {filtered.map(e => <ExpenseItem key={e.id} e={e} />)}
              </View>
            </View>
          </View>
        </ScrollView>
...
const dk = StyleSheet.create({
  ...
  leftCol:    { flex: 3, gap: 14 },
  rightCol:   { flex: 7, gap: 12 },
  card:       { borderRadius: 14, borderWidth: 1, padding: 16, gap: 12 },
  cardTitle:  { fontSize: 15 },
  catRow:     { flexDirection: "row", alignItems: "center", gap: 8, paddingVertical: 6, borderTopWidth: 1, borderTopColor: "transparent" },
  catDot:     { width: 10, height: 10, borderRadius: 5 },
  catName:    { flex: 1, fontSize: 13 },
  catVal:     { fontSize: 13 },
  filterRow:  { flexDirection: "row", alignItems: "center", flexWrap: "wrap", gap: 8, borderRadius: 12, borderWidth: 1, padding: 12 },
});
...
const styles = StyleSheet.create({
  ...
  modalOverlayCenter: { justifyContent: "center", alignItems: "center", paddingHorizontal: 24 },
  modalSheet:   { borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, gap: 10 },
  modalDialog:  { borderRadius: 20, borderTopLeftRadius: 20, borderTopRightRadius: 20 },
  ...
  dialogOverlay:{ flex: 1, backgroundColor: "#00000060", alignItems: "center", justifyContent: "center", padding: 24 },
  dialogBox:    { width: "100%", maxWidth: 380, borderRadius: 20, padding: 24, gap: 12, alignItems: "center" },
  dialogIcon:   { width: 52, height: 52, borderRadius: 26, alignItems: "center", justifyContent: "center" },
  dialogTitle:  { fontSize: 17, textAlign: "center" },
  dialogSub:    { fontSize: 13, textAlign: "center", lineHeight: 20 },
